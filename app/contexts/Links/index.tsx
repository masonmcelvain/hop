...
export const LinksContext = React.createContext<
  {state: StateType, dispatch: React.Dispatch<LinkActionTypes>}
>({
  state: InitialState,
  dispatch: () => null,
});

export const LinksProvider = ({ children }: {children: React.ReactChild}): JSX.Element => {
  ...
  React.useEffect(() => {
    chrome.storage.sync.get(null, (result) => {
      ...
      if (STORAGE.NEXT_LINK_ID in result) {
        dispatch({type: LinkAction.SET_NEXT_LINK_ID, payload: result[STORAGE.NEXT_LINK_ID]});
      } else {
      ...
      if (STORAGE.STORED_LINKS in result) {
        dispatch({type: LinkAction.SET_LINKS, payload: result[STORAGE.STORED_LINKS]});
      } else {
